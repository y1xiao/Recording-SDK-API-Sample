...
#ifndef IAgoraLinuxSdkCommon_h
#define IAgoraLinuxSdkCommon_h
 ...
 * You can perform a bitwise AND operation on the code and each enum value, and those with non-zero results are the reason for the exit.
 * For example, if you perform a bit-by-bit AND operation on code 6 (binary 110) and each enum value, only `LEAVE_CODE_SIG` (binary 10) and `LEAVE_CODE_NO_USERS` (binary 100) get a non-zero result.
 * The reasons for exiting, in this case, include a timeout and a signal triggering the exit.
 */
enum LEAVE_PATH_CODE {
    ...
    LEAVE_CODE_SIG = 1<<1,
    /** 4 (binary 100): The recording server automatically leaves the channel and
     * stops recording because the channel has no user.
     */
    LEAVE_CODE_NO_USERS = 1<<2,
    /** 8 (binary 1000): Ignore it. */
    LEAVE_CODE_TIMER_CATCH = 1<<3,
    /** 16 (binary 10000): The recording server calls the
...
enum VIDEO_FORMAT_TYPE {
    ...
    /** 1: Video frame in H.264 or H.265 format. You receive video frames from the \ref agora::recording::IRecordingEngineEventHandler::videoFrameReceived "videoFrameReceived" callback. */
    VIDEO_FORMAT_ENCODED_FRAME_TYPE = VIDEO_FORMAT_H264_FRAME_TYPE,
    /** 2: Video frame in YUV format. You receive video frames from the \ref agora::recording::IRecordingEngineEventHandler::videoFrameReceived "videoFrameReceived" callback.*/
    VIDEO_FORMAT_YUV_FRAME_TYPE = 2,
    /** 3: Video frame in JPG format. You receive video frames from the \ref agora::recording::IRecordingEngineEventHandler::videoFrameReceived "videoFrameReceived" callback.*/
    VIDEO_FORMAT_JPG_FRAME_TYPE = 3,
    /** 4: JPG file format. */
    VIDEO_FORMAT_JPG_FILE_TYPE = 4,
    /** 5: JPG file format and MP4 or WebM video.
     - Individual mode (\ref agora::recording::RecordingConfig#isMixingEnabled "isMixingEnabled" is set as `false`): MP4 or WebM video and JPG files.
     - Composite mode (\ref agora::recording::RecordingConfig#isMixingEnabled "isMixingEnabled" is set as `true`): MP4 or WebM video file for mixed streams and JPG files for individual streams.
     */
...
enum AUDIO_FORMAT_TYPE {
    ...
    /** 1: Audio frame in AAC format. You receive audio frames from the \ref agora::recording::IRecordingEngineEventHandler::audioFrameReceived "audioFrameReceived" callback. */
    AUDIO_FORMAT_AAC_FRAME_TYPE = 1,
    /** 2: Audio frame in PCM format. You receive audio frames from the \ref agora::recording::IRecordingEngineEventHandler::audioFrameReceived "audioFrameReceived" callback.*/
    AUDIO_FORMAT_PCM_FRAME_TYPE = 2,
    /** 3: Audio-mixing frame in PCM format. You receive audio frames from the \ref agora::recording::IRecordingEngineEventHandler::audioFrameReceived "audioFrameReceived" callback. */
...
struct VideoFrame {
    ...
    union {
        ...
        VideoH265Frame *h265;
        /** Video data in jpg file format. See the structure of \ref VideoJpgFile "VideoJpgFile".*/
...
enum UID_LIST_TYPE {
  UID_SUBSCRIBE_WHITE_LIST = 0,
  UID_SUBSCRIBE_BLACK_LIST,
};

}
}
#endif /* IAgoraLinuxSdkCommon_h */
